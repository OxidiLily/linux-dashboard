    ...
    <>
      ...
      >
        ...
        {/* Badge — use popover vars so text is never white-on-white.
            Titik hidup duduk tepat di tepi kanan area plot, jadi badge yang
            selalu digambar ke kanannya berakhir DI LUAR kotak chart. Di layar
            lebar itu tersamar karena jatuh di padding kartu; di layar HP ia
            keluar dari kartu lalu dipotong shell, sehingga nilainya terbaca
            separuh ("12.7" dari "12.7 kb/s"). Kalau tidak muat, badge
            dipantulkan ke kiri titik itu — tetap menempel di ujung garis. */}
        {badge && (() => {
          const lebarBadge = formatValue(liveValue).length * 7.5 + 16
          const muatDiKanan = liveDotX + 12 + lebarBadge <= innerWidth
          const x = muatDiKanan ? liveDotX + 12 : liveDotX - 12 - lebarBadge
          return (
          <g transform={`translate(${x},${liveDotY})`}>
            <rect
              fill="var(--popover)"
              height={24}
              opacity={0.95}
