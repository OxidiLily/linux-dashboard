...
// xLokal mengubah koordinat layar jadi koordinat di dalam SVG chart.
//
// Menggantikan localPoint dari @visx/event: satu dependensi untuk satu fungsi
// yang isinya pengurangan terhadap getBoundingClientRect. Chart ini tidak
// memakai viewBox, jadi tidak ada penskalaan SVG yang perlu dibalik —
// piksel layar dan piksel chart satu banding satu.
//
// Hanya sumbu X yang dikembalikan: itu satu-satunya yang dipakai kedua
// pemanggilnya.
export function xLokal(target: SVGElement, clientX: number): number | null {
  const svg = target.ownerSVGElement ?? target;
  const rect = svg.getBoundingClientRect();
  return rect.width === 0 ? null : clientX - rect.left;
}
...
export function useChartInteraction({
  ...
  const getChartX = useCallback(
    (
      ...
      const titik = "touches" in event ? event.touches[touchIndex] : event;
      if (!titik) {
        return null;
      }
      const x = xLokal(event.currentTarget, titik.clientX);
      return x === null ? null : x - margin.left;
    },
    [margin.left]
  );
