...
import { xLokal } from "./use-chart-interaction";
import {
  ChartProvider,
  type LineConfig,
  type Margin,
...
const LiveLineChartCore = memo(function LiveLineChartCore({
  ...
  const handleMouseMove = useCallback(
    (event: React.MouseEvent<SVGGElement>) => {
      const lokal = xLokal(event.currentTarget, event.clientX);
      if (lokal === null) {
        return;
      }
      const x = lokal - margin.left;
...
// Public component
// ---------------------------------------------------------------------------
// useUkuranElemen mengukur elemen lewat ResizeObserver.
//
// Menggantikan <ParentSize> dari @visx/responsive: satu dependensi untuk satu
// pengukur yang platform-nya sudah sediakan, dan render-prop-nya memaksa
// seluruh isi chart turun satu tingkat indentasi tanpa alasan.
//
// Pembacaan dijadwalkan ke frame berikutnya — mengubah tata letak di tengah
// siklus pengamatan membuat browser mengeluh "ResizeObserver loop completed
// with undelivered notifications". Pola yang sama dipakai pasangAutoFit di
// hooks/use-terminal-fit.ts. Nilai yang sama tidak di-set ulang, jadi tidak
// ada render tambahan saat ukurannya tidak berubah.
function useUkuranElemen(ref: React.RefObject<HTMLElement | null>) {
  const [ukuran, setUkuran] = useState({ width: 0, height: 0 });
  ...
  useEffect(() => {
    ...
    const pengamat = new ResizeObserver(() => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        setUkuran((lama) =>
          lama.width === rect.width && lama.height === rect.height
            ? lama
...
export function LiveLineChart({
  ...
  const ukuran = useUkuranElemen(containerRef);
