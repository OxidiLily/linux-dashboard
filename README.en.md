...
3. **The stack is started once** with `sh run.sh start --wait-timeout 600`
   ...
   System → Docker.
4. **The stack registers itself** in System → Docker. A stack created by a
   panel component does not sit in the "not registered" list waiting for a
   Register button — the panel wrote the compose file, so the panel knows
   where it is. Registration is idempotent and keyed on the compose file path
   (not the name, so a stack you renamed is still recognized), and the row
   disappears again once the component is removed. Machines that installed
   Supabase before this release fix themselves the next time the Docker page
   is opened.

The stack name **must** be `supabase`: Supabase's `docker-compose.yml` sets
`name: supabase` at the top level, and the panel derives the compose project
name from the stack name. If the two differ, one `Down` from the panel orphans
the running containers and `Up` starts a second set that fights for the ports.
...
`http://<panel host>:8000`. What greets you there is the gateway's basic-auth
box, and its password was generated by setup.sh — nobody ever typed it. So the
component card carries a note saying where the values live:
`DASHBOARD_USERNAME` and `DASHBOARD_PASSWORD` in the stack `.env`, one click
away via System → Docker → supabase → the **.env** button (on disk:
`/opt/supabase/supabase-project/.env`). The values themselves are deliberately
not printed on the card: the Components page is readable at a glance by anyone
